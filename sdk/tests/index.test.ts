...
import { type SDKFullSafeTransaction, type ChainId, SDKTransactionDetails, SDKHarbourSignature } from "../src/types";
...
describe("Harbour SDK Functions", () => {
	describe("getTransactions", () => {
		it("should correctly fetch and assemble transactions with signatures", async () => {
			...
			expect(mockRetrieveSignatures).toHaveBeenCalledWith(
				sampleOwners[0],
				sampleSafeAddress,
				sampleChainId,
				sampleNonce,
		...
		it("should not include transactions if retrieveTransaction returns stored: false", async () => {
			mockRetrieveSignatures.mockResolvedValueOnce([[{ r: "0xr1", vs: "0xvs1", txHash: sampleTxHash1 }], BigInt(1)]);
	...
	describe("enqueueTransaction", () => {
		...
		it("should throw an error if signer has no provider", async () => {
			...
			await expect(enqueueTransaction(signerWithoutProvider, sampleFullTx, sampleSignature)).rejects.toThrow(
