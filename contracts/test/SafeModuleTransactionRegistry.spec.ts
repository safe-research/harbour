...
import { Signature, TypedDataDomain, ZeroAddress } from "ethers";
import { execTransaction } from "./utils/utils";
import { loadFixture } from "@nomicfoundation/hardhat-toolbox/network-helpers";
import { SafeModuleTransactionRegistry } from "../typechain-types/contracts/SafeModuleTransactionRegistry";

describe("SafeModuleTransactionRegistry", function () {
  let masterCopy: any;
  ...
  it("Should successfully transfer native token", async function () {
    ...
    expect(
      await safeModuleTransactionRegistry.moduleTxNonces.staticCall(safe.target)
    ).to.be.equal(1n);
  });

  it("[Add tx to queue] Should successfully transfer native token", async function () {
    const { deployer, alice, safe, safeModuleTransactionRegistry } =
      await loadFixture(deployFixture);
  ...
  it("Reverts when trying to execute same tx again", async function () {
    const { alice, safe, safeModuleTransactionRegistry } = await loadFixture(
      deployFixture
    );
  ...
  it("Reverts when provided nonce is higher then expected", async function () {
    const { alice, safe, safeModuleTransactionRegistry } = await loadFixture(
      deployFixture
    );
  ...
  it("Reverts when signature is empty", async function () {
    const { alice, safe, safeModuleTransactionRegistry } = await loadFixture(
      deployFixture
    );
  ...
  it("Allow identical transaction registration for same nonce", async function () {
    const { alice, safe, safeModuleTransactionRegistry } = await loadFixture(
      deployFixture
    );
  ...
  it("Reverts when trying to re-use signature for same nonce", async function () {
    const { alice, safe, safeModuleTransactionRegistry } = await loadFixture(
      deployFixture
    );
  ...
  it("No double execution", async function () {
    const { alice, safe, safeModuleTransactionRegistry } = await loadFixture(
      deployFixture
    );
  ...
  it("Nested safe transaction execution", async function () {
    const { alice, safe, safeModuleTransactionRegistry } = await loadFixture(
      deployFixture
    );
  ...
  it("Execute transaction with 2:2 Safe", async function () {
