...
import { type BaseContract, ContractTransactionResponse, MaxUint256, type Signer, Wallet, ZeroAddress, ZeroHash } from "ethers";
import { ethers, hardhatArguments } from "hardhat";
import { EntryPoint__factory, QuotaMixin, SafeInternationalHarbour__factory, TestQuotaManager__factory, TestToken__factory } from "../typechain-types";
...
describe("QuotaManager", () => {
	async function deployFixture() {
		...
		const quotaManager = await quotaManagerFactory.deploy(buildQuotaConfig(await testToken.getAddress()));
		return { deployer, alice: alice as unknown as Signer, bob, quotaManager, quotaManagerFactory, testToken };
	}

	function calculateNextQuotaReset(updateTimestamp: bigint, prevNextReset: bigint, resetTimeframe: bigint = 24n * 3600n): bigint {
		return updateTimestamp - (updateTimestamp - prevNextReset) % resetTimeframe + resetTimeframe
	}

	async function calculateNextQuotaResetFromTx(updateTx: ContractTransactionResponse, prevNextReset: bigint, resetTimeframe: bigint = 24n * 3600n): Promise<bigint> {
		const updateBlock = await updateTx.getBlock()
		const updateTimestamp = BigInt(updateBlock!.timestamp)
		return calculateNextQuotaReset(updateTimestamp, prevNextReset, resetTimeframe)
	}

	async function signWithdrawal(wallet: Signer, quotaManager: QuotaMixin, amount: bigint, beneficiary: string, nonce: bigint = 0n): Promise<string> {
	...
	it("should revert if tokens cannot be transfered for deposit", async () => {
		...
		await expect(
			quotaManager.depositTokensForSigner(alice, ethers.parseUnits("1", 18))
	...
	it("should be able to deposit tokens for signer", async () => {
		...
		await expect(
			quotaManager.depositTokensForSigner(alice, ethers.parseUnits("1.0", 18))
		).to.emit(quotaManager, "Deposit").withArgs(alice, ethers.parseUnits("1", 18))
	...
	it.only("should revert if deposited tokens result in too high quota", async () => {
		...
		await expect(
			quotaManager.checkAndUpdateQuota(alice, 100)
		).to.be.revertedWith("Max signer quota too high")
	});
	...
	it("should be able to withdraw tokens if quota was not used", async () => {
		...
		).to.emit(quotaManager, "Withdraw").withArgs(alice, ethers.parseUnits("500", 18))
	...
	it("should revert if same signature is used twice for withdraw", async () => {
		...
		await expect(
			quotaManager.widthdrawTokensForSigner(sig, ethers.parseUnits("500", 18), bob, 0)
		).to.be.revertedWith("Withdrawal was already performed")
		expect(await quotaManager.quotaStatsForSigner(alice)).to.be.deep.eq([
			ethers.parseUnits("500", 18), // Signer Token Balance
			0n, // Used Signer Quota
			0n // Next Signer Quota Reset
	...
	it("cannot withdraw if any quota has been used", async () => {
		...
		await expect(
			quotaManager.widthdrawTokensForSigner(sig, ethers.parseUnits("1", 18), bob, 0)
		).to.be.revertedWith("Tokens have been used during this timeframe")
	});
