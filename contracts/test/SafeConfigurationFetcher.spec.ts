...
async function setupSafe(modCount = 5): Promise<TestContext> {
  ...
  const fetcher = await (
    await ethers.getContractFactory("SafeConfigurationFetcher", deployer)
  ).deploy();
  ...
  const guard = await (
    await ethers.getContractFactory("DebugTransactionGuard", deployer)
  ).deploy();
  ...
  if (!receipt)
    throw new Error("Proxy creation transaction did not return a receipt");
  const [creationEvent] = await proxyFactory.queryFilter(
  ...
  for (const m of modules) {
    await execSafeTx(
      ...
      deployer,
      safeAddress,
      safe.interface.encodeFunctionData("enableModule", [m]),
    );
  }
  // Set guard
  await execSafeTx(
    ...
    deployer,
    safeAddress,
    safe.interface.encodeFunctionData("setGuard", [guardAddress]),
  );
...
function assertSafeConfig(
  actual: SafeConfigurationStruct,
  expected: SafeConfigurationStruct,
) {
...
// Setup multiple Safe contracts for batch testing
async function setupMultipleSafes(): Promise<{
  fetcher: SafeConfigurationFetcher;
  safes: { address: string; config: SafeConfigurationStruct }[];
}> {
  const ctx1 = await setupSafe(3); // Safe with 3 modules
  const ctx2 = await setupSafe(0); // Safe with no modules

  return {
    fetcher: ctx1.fetcher,
    safes: [
      { address: ctx1.safeAddress, config: ctx1.safeConfig },
      { address: ctx2.safeAddress, config: ctx2.safeConfig },
    ],
  };
}
...
describe("SafeConfigurationFetcher", () => {
  ...
  describe("getModulesPaginated", () => {
    it("paginates modules correctly", async () => {
      const pageSize = 2;
      const [page1, next1] = await ctx.fetcher.getModulesPaginated(
        ctx.safeAddress,
        SENTINEL_MODULES,
        pageSize,
      );
      ...
      if (ctx.safeConfig.modules.length > pageSize) {
        ...
        const [page2, next2] = await ctx.fetcher.getModulesPaginated(
          ctx.safeAddress,
          next1,
          pageSize,
        );
        for (const m of page2) expect(ctx.safeConfig.modules).to.include(m);
        expect(next2).to.equal(
          ctx.safeConfig.modules.length > pageSize * 2
            ? next2
            : ethers.ZeroAddress,
        );
      } else {
    ...
    it("reverts if Safe missing", async () => {
      await expect(
        ctx.fetcher.getModulesPaginated(
          ethers.Wallet.createRandom().address,
          SENTINEL_MODULES,
          1,
        ),
  ...
  describe("getFullConfiguration", () => {
    it("fetches complete configuration and modules", async () => {
      ...
      const [full, cursor] = await ctx.fetcher.getFullConfiguration(
        ctx.safeAddress,
        3,
        2,
      );
    ...
    it("supports truncated pagination", async () => {
      ...
      for (const m of full.modules)
        expect(ctx.safeConfig.modules).to.include(m);
      if (ctx.safeConfig.modules.length > 2)
        expect(cursor).to.not.equal(ethers.ZeroAddress);
    });

    it("returns no modules if none enabled", async () => {
      const emptyCtx = await loadFixture(setupSafeNoModules);
      const [full, cursor] = await emptyCtx.fetcher.getFullConfiguration(
        emptyCtx.safeAddress,
        1,
        10,
      );
  ...
  describe("getFullConfigurationMany", () => {
    it("fetches configurations for multiple Safes", async () => {
      const multipleCtx = await loadFixture(setupMultipleSafes);
      const safeAddresses = multipleCtx.safes.map((s) => s.address);

      const [configs, cursors] =
        await multipleCtx.fetcher.getFullConfigurationMany(safeAddresses, 3, 2);

      expect(configs.length).to.equal(2);
      expect(cursors.length).to.equal(2);

      // Verify each Safe configuration
      for (let i = 0; i < configs.length; i++) {
        assertSafeConfig(configs[i], multipleCtx.safes[i].config);
        // First Safe has modules, second doesn't
        if (i === 0) {
          expect(configs[i].modules.length).to.be.greaterThan(0);
        } else {
          expect(configs[i].modules).to.be.empty;
        }
      }
    });

    it("works with a single Safe in batch", async () => {
      const [full, cursor] = await ctx.fetcher.getFullConfiguration(
        ctx.safeAddress,
        3,
        2,
      ...
      const [batchConfigs, batchCursors] =
        await ctx.fetcher.getFullConfigurationMany([ctx.safeAddress], 3, 2);

      expect(batchConfigs.length).to.equal(1);
      expect(batchCursors.length).to.equal(1);
      assertSafeConfig(batchConfigs[0], full);
      expect(batchCursors[0]).to.equal(cursor);
    });

    it("returns empty arrays for empty safes input", async () => {
      const [configs, cursors] = await ctx.fetcher.getFullConfigurationMany(
        [],
        3,
        2,
      );
      expect(configs).to.be.empty;
      expect(cursors).to.be.empty;
    });

    it("handles mixed valid and invalid Safes", async () => {
      const invalidAddress = ethers.Wallet.createRandom().address;
      await expect(
        ctx.fetcher.getFullConfigurationMany(
          [ctx.safeAddress, invalidAddress],
          3,
          2,
        ),
      ).to.be.reverted;
  ...
  describe("Edge Cases", () => {
    ...
    it("invalid pageSize = 0 errors", async () => {
      await expect(
        ctx.fetcher.getModulesPaginated(ctx.safeAddress, SENTINEL_MODULES, 0),
      ).to.be.reverted;
      await expect(ctx.fetcher.getFullConfiguration(ctx.safeAddress, 1, 0)).to
        .be.reverted;
      await expect(
        ctx.fetcher.getFullConfigurationMany([ctx.safeAddress], 1, 0),
      ).to.be.reverted;
    });
  });
