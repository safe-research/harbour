import { getAddress, parseEther, parseUnits, Wallet, type Signer } from "ethers";
import type { ActionType, EthereumProvider, TaskArguments } from "hardhat/types";
import { buildSafeTx, buildSignedUserOp, calculateMaxGasUsageForUserOp, encodePaymasterData } from "../../test/utils/erc4337";
import { EntryPoint__factory, ERC20__factory, IERC20__factory, SafeHarbourPaymaster__factory, SafeInternationalHarbour__factory } from "../../typechain-types";
...
export const submitSignature: ActionType<TaskArguments> = async (taskArgs, hre) => {
	...
	const harbourAddress = taskArgs.harbour ? getAddress(taskArgs.harbour) : (await hre.deployments.get("SafeInternationalHarbour")).address;
	...
	let paymasterAndData = undefined;
	...
	const { userOp, signature } = await buildSignedUserOp(harbour, signer, safeTxChainId, safeAddress, safeTx, paymasterAndData);
	if (taskArgs.asPaymaster) {
...
export const depositValidatorTokens: ActionType<TaskArguments> = async (taskArgs, hre) => {
	...
	const depositTx = await paymaster.depositTokensForSigner(hardhatSigner.address, amount)
